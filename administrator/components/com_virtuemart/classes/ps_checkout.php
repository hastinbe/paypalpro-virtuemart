...
/**
*
* @version $Id: ps_checkout.php 1451 2008-07-04 19:55:31Z soeren_nb $
* @package VirtueMart
* @subpackage classes
...
class ps_checkout {
	...
	function validate_form(&$d) {
		...
		//Begin MOD for Paypal Express
		//We do not need this part to activate when using Paypal Express, because Paypal has already verified it for us.
		//So, we check to see if our paypal_wpp_ex session variable is set, if so we skip this part. 
	...
	function list_payment_methods( $payment_method_id=0 ) {
		...
		//fixes checkout undefined variable error.
		//first_payment_method_id could not be filled sometimes thus throws variable undefined error
		if(!isset($first_payment_method_id))
		{
			$first_payment_method_id = $payment_method_id;
		}
...
----------------------------' 
		...
		//Check to see if our paypal checkout module has returned a payment status. 
		//If so we use it instead of the standard 'P'. If not we use the 'P' status
		if(isset($_SESSION['ps_paypal_wpp_paystatus']))
		{
			$paystatus = $_SESSION['ps_paypal_wpp_paystatus'];
		}
		else
		{
			$paystatus = 'P';
    	...
    	*/
		...
		//Unset Paypal Express if used
		if(isset($_SESSION['paypal_wpp_ex']))
		{
			unset($_SESSION['ps_paypal_wpp_paystatus']);
			unset($_SESSION['paypal_wpp_ex']);
			unset($_SESSION['paypal_ex_token']); 
			unset($_SESSION['paypal_ex_payerID']);
		}
